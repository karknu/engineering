...
The above figure compares the outcomes from the close-random policy mentioned above and a peer
scoring policy. The policy scores peers based on how often they are the first to present the
node with a new block header. Nodes start with all random links and periodically calibrate.
During this calibration, they replace the worst 20% or worst 40% peers with newly picked
random nodes.
...
periodic re-calibrations with 20% replacement lead to a progressive improvement in the overall
...
The heat map provides a rich visual representation of how these varying policies interact
